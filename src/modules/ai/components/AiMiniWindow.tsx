...
import { useAgentRouter, useSessionConfig } from "../hooks/useAgentRouter";
...
function Header({
  ...
  const sessionId = useChatStore((s) => s.activeSessionId);
  const { config, setMode, setModel } = useSessionConfig(sessionId);
  ...
  return (
    <div className="relative flex h-11 shrink-0 items-center justify-between gap-2 border-b border-border/60 px-3">
      <div className="flex min-w-0 items-center gap-1.5">
        ...
        {config?.available_modes && config.available_modes.length > 0 && (
          <ModeSelector 
            modes={config.available_modes} 
            current={config.current_mode?.mode_id} 
            onSelect={setMode} 
          />
        )}
        ...
        {config?.available_models && config.available_models.length > 0 && (
          <ModelSelector 
            ...
            current={config.current_model?.model_id} 
            onSelect={setModel} 
          />
        )}
...
function SessionPicker() {
  ...
  const isAcp = router.mode === "acp_agent";

  const legacySessions = useChatStore((s) => s.sessions);
  const activeId = useChatStore((s) => s.activeSessionId);
  const switchSession = useChatStore((s) => s.switchSession);
  const newSession = useChatStore((s) => s.newSession);
  const deleteSession = useChatStore((s) => s.deleteSession);

  const activeTitle = isAcp
    ? router.sessionList.find((s) => s.session_id === activeId)?.title || "New chat"
    : legacySessions.find((s) => s.id === activeId)?.title || "New chat";

  const sortedLegacy = [...legacySessions].sort((a, b) => b.updatedAt - a.updatedAt);
  const sortedAcp = [...router.sessionList].sort((a, b) => b.last_updated - a.last_updated);
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="min-w-56">
        ...
        {isAcp ? (
          <>
            {sortedAcp.length > 0 ? <DropdownMenuSeparator /> : null}
            {sortedAcp.map((s) => (
              <SessionRow
                key={s.session_id}
                session={{
                  ...
                  createdAt: s.last_updated,
                  updatedAt: s.last_updated,
                }}
                titleSource={s.title_source}
                active={s.session_id === activeId}
                onSelect={() => {
                  void router.loadSession(s.session_id);
                  switchSession(s.session_id);
                }}
                onDelete={() => {
                  void router.deleteSession(s.session_id);
          ...
          <>
            {sortedLegacy.length > 0 ? <DropdownMenuSeparator /> : null}
            {sortedLegacy.map((s) => (
              <SessionRow
                key={s.id}
                session={s}
...
function SessionRow({
  ...
  session: SessionMeta;
  titleSource: "user" | "agent" | "provisional" | null;
  active: boolean;
  disabled?: boolean;
  onSelect: () => void;
  onDelete: () => void;
        ...
        {titleSource === "user" && (
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="shrink-0 flex items-center text-muted-foreground opacity-50">
                <HugeiconsIcon icon={UserIcon} size={11} strokeWidth={2} />
              </span>
            </TooltipTrigger>
            <TooltipContent>Title set by you</TooltipContent>
          </Tooltip>
        )}
      </div>
      <button
