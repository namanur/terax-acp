...
export async function sendMessage(text: string): Promise<boolean> {
  ...
  const executionMode = usePreferencesStore.getState().executionMode;
  if (executionMode === "acp_agent") {
    try {
      const cwd = state.live.getCwd();
      await invoke("agent_session_send", { sessionId, message: text, cwd });
      return true;
    } catch (e) {
      console.error("agent_session_send failed", e);
      return false;
    }
  }
